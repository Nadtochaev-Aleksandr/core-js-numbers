...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const side1 = x2 - x1;
  const side2 = y2 - y1;
  const distance = (side1 ** 2 + side2 ** 2) ** 0.5;
  return distance;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const vector1Length = ((x1 - 0) ** 2 + (y1 - 0) ** 2) ** 0.5;
  const vector2Length = ((x2 - 0) ** 2 + (y2 - 0) ** 2) ** 0.5;
  const vectorsMultiplues = x1 * x2 + y1 * y2;
  const cosinusAngle = vectorsMultiplues / (vector1Length * vector2Length);
  const angle = Math.acos(cosinusAngle);
  return angle;
}
...
function toNumber(value, def) {
  const numParametr = Number(value);
  if (Number.isNaN(numParametr)) {
    return def;
  }
  return numParametr;
}
...
function toExponential(number, fractionDigits) {
  const exponentialNumbers = number.toExponential(fractionDigits);
  return exponentialNumbers.toString();
}
...
function toFixed(number, fractionDigits) {
  const mynumber = number.toFixed(fractionDigits);
...
// const resultString = num.toString(base);
